...
/**
 * Sends customer/admin/processing order emails (same logic as POST /api/orders/notify).
 * Use this from API routes instead of HTTP-fetching /api/orders/notify so serverless
 * handlers don't terminate before the outbound request completes.
 */
export async function sendOrderNotification(params: {
  ...
  const orderRef =
    sanitizedOrderData.orderNumber ||
    sanitizedOrderData.orderId.slice(0, 8).toUpperCase();
