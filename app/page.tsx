  ...
  return (
    <main>
      ...
      <section style={{ padding: '48px 0' }}>
        <div className="container">
          <h2 style={{ fontSize: '1.6rem', fontWeight: 700, textAlign:'center', marginBottom: 24 }}>Featured Products</h2>
          <div className="grid">
            {products?.map((p) => (
              <Link key={p.id} href={`/products/${p.id}`} className="card" style={{ padding: 16 }}>
                ...
                <h3 style={{ marginTop: 12, marginBottom: 6, color: 'var(--accent)' }}>{p.name}</h3>
                <p style={{ color: 'var(--muted)', fontSize: '.95rem', margin: 0 }}>{p.description}</p>
